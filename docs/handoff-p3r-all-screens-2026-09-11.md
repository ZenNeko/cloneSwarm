...
### ~~ที่ต้องระวัง — บั๊กจริงที่ยังไม่แก้~~ → **แก้แล้ว · ตรวจซ้ำ 2026-09-11**
เอกสารรุ่นก่อนเขียนว่า `TalentShopUI` อยู่บน `Canvas` ไม่ใช่บน panel
ตรวจ YAML ของ `MenuScene` แล้ว — มันอยู่บน `P3R_TalentShop` เรียบร้อย ไล่ตั้งแต่ตอนย้ายจอ P3R
ลงซีนจริง · ทั้งเรื่อง lifecycle และเรื่องเงินหาย (ซึ่ง `SaveAutoFlush` ก็กันไว้อีกชั้นอยู่แล้ว) จบทั้งคู่
...
## คำถามที่ยังไม่ได้ตัดสิน — **ปิดครบทั้งสี่ข้อแล้ว · 2026-09-11**

**1 · ลุค ALL-CAPS กับภาษาไทย → ทำไปแล้วตั้งแต่ก่อนหน้านี้ เอกสารค้าง**
`P3RThaiTracking` ชดเชยทั้ง **ระยะถ่าง** (ไทยได้ 0) และ **ขนาด** (`thaiFontScale` 1.08) ให้เอง
ตัวเฝ้าติดอยู่จริง 23 ช่องใน `MenuScene` · 117 ช่องใน `SampleScene` (ยืนยันจาก YAML ของซีน)
การตัดสินใจอยู่ที่ **ตัวช่องเอง** ไม่ใช่ที่ผู้เขียนข้อความ จึงไม่มีกฎให้ใครลืม
ข้อยกเว้นที่จงใจ: `P3RMenuItem` ไม่ชดเชยขนาด เพราะ `rowPitch` คงที่ ขยาย 8% แล้วแถวชนกัน

**2 · MAIN · A หรือ B → เลือก A (แถบกอดคำ)**
`P3RMenuItem.barHugsLabel` · เมนูหลักเปิด · จอ pause ปิด (แถบที่นั่นเป็นสแลบขนาดตายตัวที่มีเลขลำดับข้างใน)
เหตุผล: `TALENT SHOP` ยาวเกิน `barExtendLeft` (320) จนยื่นออกนอกแถบตัวเอง ซึ่งอ่านเป็น
"ของพัง" ไม่ใช่สไตล์ · และความยาวคำเปลี่ยนทุกครั้งที่สลับภาษา ค่าคงที่จึงจูนให้ถูกทุกภาษาไม่ได้

**ไม่ใช่ A แบบกอดเปลือย** — ความกว้างเดิมยังเป็น **พื้น** (`Mathf.Max`) คำสั้นอย่าง `PLAY`
จะได้ไม่กลายเป็นแถบกุด และทุกรายการที่วันนี้พอดีอยู่แล้วหน้าตาไม่เปลี่ยนเลย
`P3RSmokeTest.CheckMenuBarsCoverLabels` วัดเป็นตัวเลขไว้แล้วว่าไม่มีคำไหนยื่นออกนอกแถบ

**3 · CONFIG ปุ่มแบ่งช่องหรือ dropdown → ทำไปแล้ว เอกสารค้าง**
`P3RSegmentedControl` วาดปุ่มแบ่งช่องตามแบบ โดยมี `TMP_Dropdown` **ซ่อนไว้เป็นตัวถือตรรกะเดิม**
`SettingsMenuUI` ไม่ถูกแตะเลย — ได้หน้าตาตามแบบ และยังรองรับจำนวนตัวเลือกเท่าไรก็ได้

**4 · Title เป็นซีนแยกหรือ panel → เป็น panel ใน `MenuScene`**
`MenuManager.titlePanel` เข้ากลุ่ม `ShowPanel()` ปกติ · `Start()` เปิด Title ก่อนถ้าช่องนี้มีค่า
ปล่อยช่องว่างไว้ = พฤติกรรมเดิมทุกอย่าง (เข้า Main เลย) จึงย้อนกลับได้โดยไม่ต้องแก้โค้ด
