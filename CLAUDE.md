...
**Swarm Survivors** (working title "Clone Swarm") — Unity 6 (`6000.7.0b1`) top-down 1–4-player co-op bullet-heaven survival roguelike. Inspired by LoL Swarm / Vampire Survivors / Brotato. Built on **Netcode for GameObjects (NGO)**. Win condition: kill the Main Boss that spawns at the 15-minute mark.
...
- **Open project**: Unity Hub → add `E:/Zenity Why not/cloneSwarm` → open with the version in `ProjectSettings/ProjectVersion.txt` (currently `6000.7.0b1`) — don't hardcode it anywhere, the project has been moved across 6000.7 alphas/betas several times
- **Scenes**: `Assets/GameScenes/MenuScene.unity` (lobby/character select) → `Assets/GameScenes/SampleScene.unity` (gameplay). There is no `Assets/Scenes/` directory. `SampleScene.unity` is the only real gameplay scene — `SampleScene black.unity` and `Scene 2.unity` are near-empty stubs and `WeaponTestScene.unity` is a dev harness; all three are still listed in Build Settings but disabled (`enabled: 0` in `ProjectSettings/EditorBuildSettings.asset`)
...
That is the target, and the rule for **new** gameplay state. **Authority as it actually is today** is looser — don't assume the server is the source of truth for these:

- **Combat values are client-computed.** The owner's weapon/ability scripts roll damage, crit, radius and hit centre, then pass them as arguments to the `PlayerWeaponManager.<Xxx>ServerRpc` methods (e.g. `FireMeleeServerRpc(center, radius, damage, isCrit, …)`). The server applies what it is sent, within sanity clamps only — it does not recompute from the player's stats.
- **Player progression is owner-first, mirrored to the server.** Weapons, stats and augments are applied locally on the owner, then replayed on the server by an event RPC (`AddWeaponServerRpc(weaponName)` / `UpgradeWeaponServerRpc`, `PlayerStatManager.ApplyStatServerRpc(StatType)`, `PlayerAugmentManager.AcquireServerRpc(index)`). They are not held in server-written `NetworkVariable`s, so a reconnecting client has nothing to restore from. Moving this to server-held state is "Stage 1" of `docs/plan-server-state-and-reconnect.md`.
...
- **Rolls** (`RollDefinition` / `RollContext`) — seeded per-fight randomization so the same action plays out mirrored / rotated / on a different victim each pull. **`BossController.AttackLoop` rolls only top-level actions in the phase list**; `ComboAction` / `BossTimelineAction` must call `RollForSubActions(...)` **once** at start and their children then `Peek` — re-rolling per child breaks a pattern into inconsistent pieces. Spatial kinds (`Anchor` / `SnapAngle` / `MirrorX` / `MirrorZ`) are applied generically in `SpawnAoEActionBase` via `RollTransform`; pivot is `BossEncounterConfig.arena.center` (falls back to the boss position **with a warning**) — but only for arena-relative targeting (`StaticCoords` / `ArenaAnchor`); boss/player-anchored modes stay on that entity and only `targetOffset` is rotated (`RollTransform.ApplyVector`). Roll once per volley and apply the same transform to every spawn position.
...
- `BossManager` — spawns mini bosses from `MapData.miniBosses` (id = `TimelineCue.variant`, empty = random; per-tier `miniBossOverrides` swap the config) and falls back to the scene `miniBossPrefabs[]` when a map has no roster. Boss/mini-boss HP = prefab × `DifficultyProfile.bossHpMult` × `HpForPlayers(count at spawn)`
- **Difficulty** — numbers live in one `DifficultyProfile` per tier under `Resources/Difficulty/` (shared by every map; missing file = neutral ×1). Harder *mechanics* are timeline clips with `limitTiers` (`minTier`/`maxTier`), not a copied config. `MapData.TierContent` holds map content only (schedule, waves, music, overrides)
